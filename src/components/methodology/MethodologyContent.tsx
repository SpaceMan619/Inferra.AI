...
/**
 * The methodology content, rendered in two places.
 *
 * "page"  — the public route at /methodology, for someone who is not signed
 *           in. Carries its own masthead and a way back to the landing page.
 * "panel" — inside the dashboard, where the sidebar and title bar already
 *           exist, so the masthead would be duplicate chrome.
 *
 * One copy of the content either way; only the framing differs.
 */
export default function MethodologyContent({
  ...
  return (
    <div className={isPage ? "bg-[#f7f7f5] text-[#222f30]" : "text-[#222f30]"}>
      {isPage ? (
        ...
        <div className="mx-auto max-w-[1100px] px-[clamp(4px,2vw,12px)]">
          <p className="max-w-[56ch] text-[17px] font-light leading-[1.55] text-[#445e5f]">
            Every number here carries its source, the wording that supports it,
            and the period it describes. This is how to check us.
          </p>
        </div>
      )}
      ...
      <section className={`mx-auto mt-[clamp(40px,6vw,72px)] max-w-[1100px] ${isPage ? "px-[clamp(20px,5vw,50px)]" : "px-[clamp(4px,2vw,12px)]"}`}>
      ...
      {/* ── 01 What we measure ───────────────────── */}
      <section className={`mx-auto mt-[clamp(56px,9vw,120px)] max-w-[1100px] ${isPage ? "px-[clamp(20px,5vw,50px)]" : "px-[clamp(4px,2vw,12px)]"}`}>
        <Reveal>
          <SectionHead
      ...
      {/* ── Limits ───────────────────────────────── */}
      <section className={`mx-auto mt-[clamp(56px,9vw,120px)] max-w-[1100px] pb-[clamp(48px,8vw,112px)] ${isPage ? "px-[clamp(20px,5vw,50px)]" : "px-[clamp(4px,2vw,12px)]"}`}>
        <Reveal>
          <SectionHead
